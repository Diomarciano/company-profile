        ...
        <div className="max-w-6xl mx-auto">
          ...
          {products.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {products.map((p: any) => (
                <Link href={`/produk/${p.slug?.current}`} key={p.slug?.current} className="bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100 hover:shadow-md hover:-translate-y-1 transition-all duration-200 block">
                  {p.image ? (
                    <div className="relative h-48 w-full">
                      <Image src={urlFor(p.image).width(600).height(400).url()} alt={p.name} fill className="object-cover" />
                    </div>
                  ) : (
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {posts.map((post: any) => (
                <Link href={`/blog/${post.slug?.current}`} key={post.slug?.current} className="border border-gray-100 rounded-2xl overflow-hidden hover:shadow-md transition-shadow duration-200 block">
                  {post.image ? (
                    <div className="relative h-48 w-full">
                      <Image src={urlFor(post.image).width(600).height(400).url()} alt={post.title} fill className="object-cover" />
                    </div>
                  ) : (
                    <div className="bg-gradient-to-br from-blue-50 to-gray-100 h-48" />
                  )}
                  <div className="p-5">
